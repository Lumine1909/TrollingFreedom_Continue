    ...
    public void Snowman(Player p) {
        ...
        scheduler.scheduleSyncDelayedTask((Core.instance), new Runnable() {
            public void run() {
                ...
                s.getWorld().spawnParticle(Particle.FIREWORK, s.getLocation(), 1, 10, 1, 50);
            }
        }, 40L);
        scheduler.scheduleSyncDelayedTask((Core.instance), new Runnable() {
            public void run() {
        ...
        scheduler.scheduleSyncDelayedTask((Core.instance), new Runnable() {
            public void run() {
                s.remove();
                s.getWorld().spawnParticle(Particle.FIREWORK, s.getLocation(), 2, 2, 2, 2);
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask((Core.instance), () -> {
            for (int x = 0; x < 250; x++) {
                ...
                Bukkit.getScheduler().runTaskLater(Core.instance, () -> {
                    s.getWorld().spawnParticle(Particle.FIREWORK, s.getLocation(), 1, 10, 1, 100);
